    ...
    public static List<Record> getDiseaseKG(String diseaseId) {
        ...
        try {
            ...
            try (Session session = driver.session()){
                Result recordListResult = session.readTransaction(tx ->
                        tx.run(
                                "match p=(n:" + EMedicalLabel.Disease.name() + "{id:$id})-[r]->(m) return p"
                                , Values.parameters("id", diseaseId)
                        ));
                recordList = recordListResult.list();
                recordListResult.consume();
                Result recordListResult2 = session.readTransaction(tx ->
                        tx.run("match p=(m)-[r]->(n:" + EMedicalLabel.Disease.name() + "{id:$id}) return p", Values.parameters("id", diseaseId)));
                recordList.addAll(recordListResult2.list());
                recordListResult2.consume();
            }
        } finally {
