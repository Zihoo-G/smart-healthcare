...
public class NeoUtil {
        ...
        try {
            ...
            try(Session session = driver.session()) {
                session.writeTransaction(tx -> tx.run("call apoc.periodic.iterate(" +
                        "'call apoc.load.csv(\"" + "file:" + file + "\",{header:true,sep:\",\",ignore:[\"label\"]}) yield map as row return row'," +
                        "'create(p:" + nodeLabel +") set p=row'," +
                        "{batchSize:1000,iterateList:true, parallel:true})").consume());
            } catch (Exception e) {
                e.printStackTrace();
            }
        } finally {
    ...
    public static boolean buildOnlyIdx(String nodeLabel, String property) {
        ...
        try {
            driver = NeoPoolUtil.borrowDriver();
            try (Session session = driver.session()) {
                summary = session.writeTransaction(tx -> tx.run("create constraint for (s:" + nodeLabel + ") require s." + property + " is unique").consume());
            } catch (Exception e) {
                e.printStackTrace();
            }
        } finally {
    ...
    public static boolean buildIndex(String nodeLabel, String property) {
        ...
        try {
            driver = NeoPoolUtil.borrowDriver();
            try (Session session = driver.session()){
                summary = session.writeTransaction(tx -> tx.run("create index on :" + nodeLabel + "(" + property + ")" ).consume());
            } catch (Exception e) {
                e.printStackTrace();
            }
        } finally {
        ...
        try {
            ...
            try (Session session = driver.session()) {
                summary = session.writeTransaction(tx -> tx.run("call apoc.periodic.iterate(" +
                        ...
                        "{batchSize:1000,iterateList:true, parallel:false});").consume()); //这里注意parallel为false，保证在创建节点之间关系时不会产生死锁问题
            } catch (Exception e) {
                e.printStackTrace();
            }
        } finally {
        ...
        try {
            ...
            try (Session session = driver.session()) {
                summary = session.writeTransaction(tx -> tx.run("call apoc.periodic.iterate(" +
                        ...
                        "{batchSize:1000, iterateList:true, parallel:false});").consume());
            } catch (Exception e) {
                e.printStackTrace();
            }
        } finally {
