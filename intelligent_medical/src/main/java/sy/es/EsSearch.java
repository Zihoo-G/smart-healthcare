...
public class EsSearch {
    ...
    public static List<Map<String, Object>> diseaseNameSearch(RestHighLevelClient client, String index, String query, int currentPage, int size) {
        SearchRequest request = new SearchRequest("disea_comple");
        SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
        BoolQueryBuilder boolQueryBuilder = QueryBuilders.boolQuery();
        boolQueryBuilder.must(QueryBuilders.matchPhraseQuery("disease_completion", query));
        searchSourceBuilder.query(boolQueryBuilder);
        searchSourceBuilder.from((currentPage - 1) * size);
        searchSourceBuilder.size(size);
        searchSourceBuilder.fetchSource(new String[]{"disease_completion", "_id"}, null);
    ...
    public static Map<String, Object> searchByDisease(RestHighLevelClient client, String index, String disease) {
        SearchRequest request = new SearchRequest(index);

        SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
        BoolQueryBuilder boolQueryBuilder = QueryBuilders.boolQuery();
        boolQueryBuilder.must(QueryBuilders.matchQuery("name", disease));
        searchSourceBuilder.query(boolQueryBuilder);
